    ...
    *model = {};
    ...
    for (uint32_t i = 0; i < gltf_data->images_count; ++i)
    {
                ...
                strcmp(mime_type, "image/jpeg") == 0)
        {
            ...
            uint8_t *buffer_data = (uint8_t*)buffer->data + gltf_image->buffer_view->offset;

            int32_t width = 0;
            int32_t height = 0;
            int32_t n_channels = 0;

            uint8_t *image_data = stbi_load_from_memory(buffer_data, (int)size, &width, &height, &n_channels, 4);
            if (!image_data)
            {
                return NULL;
            }

            assert(width > 0 && height > 0 && n_channels > 0);

            /* uint32_t mip_count = (uint32_t)floor(log2((float)(width > height ? width : height))) + 1; */
        ...
        {
            printf("Unsupported image format for GLTF model: %s\n", gltf_image->mime_type);
            assert(0);
        }
    }
    ...
    for (size_t i = 0; i < gltf_data->materials_count; ++i)
    {
        ...
        *mat = MaterialDefault(engine);
        ...
        cgltf_texture *gltf_albedo_texture = gltf_material.pbr_metallic_roughness.base_color_texture.texture;
        ...
        cgltf_texture *gltf_mr_texture = gltf_material.pbr_metallic_roughness.metallic_roughness_texture.texture;
        ...
        {
            size_t image_index = (size_t)(gltf_occlusion_texture->image - gltf_data->images);
        ...
        {
            size_t image_index = (size_t)(gltf_emissive_texture->image - gltf_data->images);
        ...
        for (size_t j = 0; j < gltf_mesh->primitives_count; ++j)
        {
            ...
            for (size_t k = 0; k < gltf_primitive->attributes_count; ++k)
            {
                ...
                case cgltf_attribute_type_position:
                {
                    ...
                    pos_buffer = ((uint8_t*)view->buffer->data) + accessor->offset + view->offset;
                ...
                case cgltf_attribute_type_normal:
                {
                    ...
                    normal_buffer = ((uint8_t*)view->buffer->data) + accessor->offset + view->offset;
                ...
                case cgltf_attribute_type_tangent:
                {
                    ...
                    tangent_buffer = ((uint8_t*)view->buffer->data) + accessor->offset + view->offset;
                ...
                case cgltf_attribute_type_texcoord:
                {
                    ...
                    uv0_buffer = ((uint8_t*)view->buffer->data) + accessor->offset + view->offset;
            ...
            if (has_indices)
            {
                ...
                uint8_t *data_ptr = ((uint8_t*)buffer->data) + accessor->offset + buffer_view->offset;
    ...
    rgBufferUpload(device, transfer_cmd_pool, model->vertex_buffer, 0, vertex_buffer_size, &vertices[0]);
    rgBufferUpload(device, transfer_cmd_pool, model->index_buffer, 0, index_buffer_size, &indices[0]);
